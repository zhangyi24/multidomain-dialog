			...
			if response_mode == "usual":
				...
				if response_policy == "inform_major_key":
					...
					if len(sys_values) < 3:
						...
						response_policy = round["response_policy"]
						content = {}
						user_act = {response_policy: content, "request": {}}
					else:
						content = {}
					...
					if len(sys_values) < 3:
						...
						response_policy = round["response_policy"]
						content = {}
						user_act = {response_policy: content, "confirm": {}}
					else:
						content = {}
				...
				if sys_act_type == "request" and response_mode == "usual":
					...
					if user_slot == "主演":
						if values["LIKE"] == [] and values["DISLIKE"] == []:
							...
							nl_candidate = ["%s。", "喜欢%s。", "喜欢看%s演的电影。", "想看%s演的电影。", "来几部%s的电影。"]
						...
						if values["LIKE"] == [] and values["DISLIKE"] == []:
							...
							nl_candidate = ["%s。", "喜欢%s。", "喜欢看%s导演的电影。", "想看%s拍的电影。", "来几部%s的电影。"]
							nl += random.choice(nl_candidate) % list2str(values["LIKE"])
						else:
							for sentiment in values:
								if values[sentiment]:
						...
						if values["LIKE"] == [] and values["DISLIKE"] == []:
							...
							if values["LIKE"]:
								nl_candidate = ["%s年代的。", "喜欢%s年代的。", "喜欢看%s年代的电影。", "想看%s年代的电影。", "来几部%s年代的电影。"]
								nl += random.choice(nl_candidate) % list2str(values["LIKE"])
						else:
							for sentiment in values:
								if values[sentiment]:
