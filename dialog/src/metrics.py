...
with tf.Graph().as_default():
	...
	with tf.Session() as sess:
		...
		# 		if list(sys_act.values())[0] == {}:
		# 			sys_domain = None
		# 		else:
		# 			sys_domain = list(list(sys_act.values())[0].keys())[0]
		# 		last_domain = sys_domain
		#
		# 		sys_act = current_round['sys_act']
		# 		domain = None
		# 		detected_user_acts = []
		...
		# 	num_correct = 0
		# 	num_dialog = 0
		# 	for round_id, current_round in enumerate(rounds_dialog):
		# 		domain_evaluated = domain_eval(current_round)
		#
		# 		if domain_evaluated == current_round['user_domain']:
		# 			num_correct += 1
		# 		else:
		# 			print(num_dialog, ': ', current_round['user_domain'], domain_evaluated, end=' ')
		# 			print('wrong')
		# 			print(current_round['sys_nl'])
		# 			print(current_round['user_nl'])
		# 		num_dialog += 1
		# 	print(num_correct, num_dialog)
		# 	print(num_correct / num_dialog)
		# domain_metrics()
	
		pass    # request_metrics
		def request_metrics():
			def request_eval(current_round):
				domain = current_round['user_domain']
				def evaluate_request_slot(current_round, domain, requested_slots):
					for slot in request_slots[domain]:
						...
						prediction = sess.run(request_slot_tracker[domain][slot]._probability,
						                      feed_dict={
							                      request_slot_tracker[domain][slot]._x_user_nl: num_request_slot[
								                      'user_nl'],
							                      request_slot_tracker[domain][slot]._x_stringmatch: num_request_slot[
								                      'stringmatch'],
			...
			num_correct = 0
			num_dialog = 0
			for round_id, current_round in enumerate(rounds_dialog[19995:19996]):
				requested_slots = request_eval(current_round)
				domain = current_round['user_domain']
				requested_answer = {domain: []}
				if 'request' in current_round['user_act']:
					requested_answer[domain].extend(current_round['user_act']['request'][domain])
					num_dialog += 1
				if 'confirm' in current_round['user_act']:
					requested_answer[domain].extend(current_round['user_act']['confirm'][domain])
					num_dialog += 1
				if requested_answer[domain]:
					if requested_answer == requested_slots:
						num_correct += 1
					else:
						print(num_dialog, ': ', requested_answer, requested_slots, end=' ')
						print('wrong')
						print(current_round['sys_nl'])
						print(current_round['user_nl'])
			print(num_correct, num_dialog)
		...
		# 				                      feed_dict={
		# 					                      inform_slot_tracker[domain][slot]._x_user_nl: num_inform_slot[
		# 						                      'user_nl'],
		# 					                      inform_slot_tracker[domain][slot]._x_stringmatch: num_inform_slot[
		# 						                      'stringmatch'],
		# 					                      inform_slot_tracker[domain][slot]._x_sys_act: num_inform_slot[
		# 						                      'sys_act'],
		...
		# 						                      value_tracker[domain][slot]._x_stringmatch: num_value[
		# 							                      'stringmatch'],
		...
		# 						informed_slots[domain].pop(slot)
		# 				elif informed_slots[domain][slot] == "NOT_MENTIONED":
		# 					informed_slots[domain].pop(slot)
		# 				elif informed_slots[domain][slot] == "DONT_CARE":
		# 					informed_slots[domain][slot] = {}
		...
		# 	num_dialog = 0
		# 	for round_id, current_round in enumerate(rounds_dialog):
		# 		informed_slots = inform_eval(current_round)
		# 		domain = current_round['user_domain']
		# 		informed_answer = {domain: {}}
		# 		if 'inform_slot' in current_round['response_policy']:
		# 			informed_answer[domain].update(current_round['user_act']['inform'][domain])
		# 		if informed_answer == informed_slots:
		# 			num_correct += 1
		# 		else:
		# 			print(num_dialog, ': ', informed_answer, informed_slots, end=' ')
		# 			print('wrong')
		# 			print(current_round['sys_nl'])
		# 			print(current_round['user_nl'])
		# 		num_dialog += 1
