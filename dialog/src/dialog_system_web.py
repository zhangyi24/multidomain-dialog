		...
		for word in entity_set:
			if len(word) > 1:
				if word in sentence:
					...
					sentence = (' ' + word + ' ').join(
						[entity_filter(sub_sentence, entity_set) for sub_sentence in sub_sentences])
					break
		return sentence
...
class DialogSystem(object):
		...
		with tf.Graph().as_default():
			with tf.name_scope("DomainTracker"):
				with tf.variable_scope("d_movie"):
					self.domain_tracker_movie = DomainTracker()
				with tf.variable_scope("d_music"):
					self.domain_tracker_music = DomainTracker()
				with tf.variable_scope("d_weather"):
					self.domain_tracker_weather = DomainTracker()
				with tf.variable_scope("d_time"):
					self.domain_tracker_time = DomainTracker()
			with tf.name_scope('RequestSlotTracker'):
				with tf.name_scope('movie'):
					with tf.variable_scope('r_s_actor'):
						self.request_slot_tracker_actor = RequestSlotTracker()
					with tf.variable_scope('r_s_director'):
						self.request_slot_tracker_director = RequestSlotTracker()
					with tf.variable_scope('r_s_genre'):
						self.request_slot_tracker_genre = RequestSlotTracker()
					with tf.variable_scope('r_s_area'):
						self.request_slot_tracker_area = RequestSlotTracker()
					with tf.variable_scope('r_s_score'):
						self.request_slot_tracker_score = RequestSlotTracker()
					with tf.variable_scope('r_s_era'):
						self.request_slot_tracker_era = RequestSlotTracker()
					with tf.variable_scope('r_s_year'):
						self.request_slot_tracker_year = RequestSlotTracker()
					with tf.variable_scope('r_s_payment'):
						self.request_slot_tracker_payment = RequestSlotTracker()
					with tf.variable_scope('r_s_length'):
						self.request_slot_tracker_length = RequestSlotTracker()
					with tf.variable_scope('r_s_intro'):
						self.request_slot_tracker_intro = RequestSlotTracker()
				with tf.name_scope('music'):
					with tf.variable_scope('r_s_artist'):
						self.request_slot_tracker_artist = RequestSlotTracker()
					with tf.variable_scope('r_s_album'):
						self.request_slot_tracker_album = RequestSlotTracker()
					with tf.variable_scope('r_s_style'):
						self.request_slot_tracker_style = RequestSlotTracker()
					with tf.variable_scope('r_s_musicera'):
						self.request_slot_tracker_music_era = RequestSlotTracker()
					with tf.variable_scope('r_s_publish'):
						self.request_slot_tracker_publish = RequestSlotTracker()
					with tf.variable_scope('r_s_musiclength'):
						self.request_slot_tracker_music_length = RequestSlotTracker()
				with tf.name_scope('weather'):
					with tf.variable_scope('r_s_weather'):
						self.request_slot_tracker_weather = RequestSlotTracker()
				with tf.name_scope('time'):
					with tf.variable_scope('r_s_date'):
						self.request_slot_tracker_date = RequestSlotTracker()
					with tf.variable_scope('r_s_week'):
						self.request_slot_tracker_week = RequestSlotTracker()
					with tf.variable_scope('r_s_time'):
						self.request_slot_tracker_time = RequestSlotTracker()
			with tf.name_scope('InformSlotTracker'):
				with tf.name_scope('movie'):
					with tf.variable_scope('i_s_actor'):
						self.inform_slot_tracker_actor = InformSlotTracker()
					with tf.variable_scope('i_s_director'):
						self.inform_slot_tracker_director = InformSlotTracker()
					with tf.variable_scope('i_s_genre'):
						self.inform_slot_tracker_genre = InformSlotTracker()
					with tf.variable_scope('i_s_area'):
						self.inform_slot_tracker_area = InformSlotTracker()
					with tf.variable_scope('i_s_era'):
						self.inform_slot_tracker_era = InformSlotTracker()
					with tf.variable_scope('i_s_payment'):
						self.inform_slot_tracker_payment = InformSlotTracker()
				with tf.name_scope('music'):
					with tf.variable_scope('i_s_artist'):
						self.inform_slot_tracker_artist = InformSlotTracker()
					with tf.variable_scope('i_s_album'):
						self.inform_slot_tracker_album = InformSlotTracker()
					with tf.variable_scope('i_s_style'):
						self.inform_slot_tracker_style = InformSlotTracker()
					with tf.variable_scope('i_s_musicera'):
						self.inform_slot_tracker_music_era = InformSlotTracker()
			with tf.name_scope('ValueTracker'):
				with tf.name_scope('movie'):
					with tf.variable_scope('v_actor'):
						self.value_tracker_actor = ValueTracker()
					with tf.variable_scope('v_director'):
						self.value_tracker_director = ValueTracker()
					with tf.variable_scope('v_genre'):
						self.value_tracker_genre = ValueTracker()
					with tf.variable_scope('v_area'):
						self.value_tracker_area = ValueTracker()
					with tf.variable_scope('v_era'):
						self.value_tracker_era = ValueTracker()
					with tf.variable_scope('v_payment'):
						self.value_tracker_payment = ValueTracker()
				with tf.name_scope('music'):
					with tf.variable_scope('v_artist'):
						self.value_tracker_artist = ValueTracker()
					with tf.variable_scope('v_album'):
						self.value_tracker_album = ValueTracker()
					with tf.variable_scope('v_style'):
						self.value_tracker_style = ValueTracker()
					with tf.variable_scope('v_musicera'):
						self.value_tracker_music_era = ValueTracker()
			with tf.name_scope('ActTracker'):
				with tf.variable_scope('first'):
					self.act_tracker_first = ActTracker()
				with tf.variable_scope('second'):
					self.act_tracker_second = ActTracker()
				with tf.variable_scope('third'):
					self.act_tracker_third = ActTracker()
				with tf.variable_scope('last'):
					self.act_tracker_last = ActTracker()
				with tf.variable_scope('other'):
					self.act_tracker_other = ActTracker()
				with tf.variable_scope('affirm'):
					self.act_tracker_affirm = ActTracker()
				with tf.variable_scope('deny'):
					self.act_tracker_deny = ActTracker()
					
			self.sess = tf.Session()
			self.sess.run(tf.global_variables_initializer())
			saver = tf.train.Saver()
			saver.restore(self.sess, "model/tmp_final/model.ckpt")
			
			self.domain_tracker = {
				"电影": self.domain_tracker_movie,
				"音乐": self.domain_tracker_music,
				"天气": self.domain_tracker_weather,
				"时间": self.domain_tracker_time
			}
			self.request_slot_tracker = {
				"电影": {
					'主演': self.request_slot_tracker_actor,
					'导演': self.request_slot_tracker_director,
					'类型': self.request_slot_tracker_genre,
					'地区': self.request_slot_tracker_area,
					'评分': self.request_slot_tracker_score,
					'年代': self.request_slot_tracker_era,
					'上映日期': self.request_slot_tracker_year,
					'资费': self.request_slot_tracker_payment,
					'片长': self.request_slot_tracker_length,
					'简介': self.request_slot_tracker_intro
				},
				"音乐": {
					'歌手': self.request_slot_tracker_artist,
					'专辑': self.request_slot_tracker_album,
					'曲风': self.request_slot_tracker_style,
					'年代': self.request_slot_tracker_music_era,
					'发行日期': self.request_slot_tracker_publish,
					'时长': self.request_slot_tracker_music_length
				},
				"天气": {
					'天气': self.request_slot_tracker_weather
				},
				"时间": {
					'日期': self.request_slot_tracker_date,
					'星期': self.request_slot_tracker_week,
					'时刻': self.request_slot_tracker_time
				}
			}
			self.inform_slot_tracker = {
				'电影': {
					'主演': self.inform_slot_tracker_actor,
					'导演': self.inform_slot_tracker_director,
					'类型': self.inform_slot_tracker_genre,
					'地区': self.inform_slot_tracker_area,
					'年代': self.inform_slot_tracker_era,
					'资费': self.inform_slot_tracker_payment
				},
				'音乐': {
					'歌手': self.inform_slot_tracker_artist,
					'专辑': self.inform_slot_tracker_album,
					'曲风': self.inform_slot_tracker_style,
					'年代': self.inform_slot_tracker_music_era
				}
			}
			self.value_tracker = {
				'电影': {
					...
					'类型': self.value_tracker_genre,
					'地区': self.value_tracker_area,
					'年代': self.value_tracker_era,
					'资费': self.value_tracker_payment
				},
				'音乐': {
					'歌手': self.value_tracker_artist,
					'专辑': self.value_tracker_album,
					'曲风': self.value_tracker_style,
					'年代': self.value_tracker_music_era
				}
			}
			self.act_tracker = {
				'first': self.act_tracker_first,
				'second': self.act_tracker_second,
				'third': self.act_tracker_third,
				'last': self.act_tracker_last,
				'other': self.act_tracker_other,
				'affirm': self.act_tracker_affirm,
				'deny': self.act_tracker_deny
			}
	...
	def evaluate_user_act(self, current_round, detected_user_acts):
		for act in user_acts:
			current_round['user_act'] = act
			num_act = data2num_act([current_round])
			prediction = self.sess.run(self.act_tracker[act]._probability,
			                           feed_dict={
				                           self.act_tracker[act]._x_user_nl: num_act['user_nl'],
				                           self.act_tracker[act]._x_stringmatch: num_act['stringmatch'],
				                           self.act_tracker[act]._is_training: False}
			                           )
			del num_act
	...
	def evaluate_domain(self, current_round, last_domain):
		domain_detected = {}
		for domain in domain_set:
			current_round['domain'] = domain
			num_domain = data2num_domain([current_round])
			prediction = self.sess.run(self.domain_tracker[domain]._probability,
			                      feed_dict={
				                      self.domain_tracker[domain]._x_user_nl: num_domain['user_nl'],
				                      self.domain_tracker[domain]._x_stringmatch: num_domain['stringmatch'],
				                      self.domain_tracker[domain]._x_sys_act: num_domain['sys_act'],
				                      self.domain_tracker[domain]._is_training: False}
			                      )
			del num_domain
	...
	def evaluate_request_slot(self, current_round, domain, requested_slots):
		for slot in request_slots[domain]:
			current_round['domain'] = domain
			current_round['slot'] = slot
			num_request_slot = data2num_request_slot([current_round])
			prediction = self.sess.run(self.request_slot_tracker[domain][slot]._probability,
			                      feed_dict={
				                      self.request_slot_tracker[domain][slot]._x_user_nl: num_request_slot[
					                      'user_nl'],
				                      self.request_slot_tracker[domain][slot]._x_stringmatch: num_request_slot[
					                      'stringmatch'],
				                      self.request_slot_tracker[domain][slot]._is_training: False}
			                      )
	...
	def evaluate_major_key(self, current_round, domain, major_key_mem):
	...
	def evaluate_inform_slot(self, current_round, domain, informed_slots):
		for slot in inform_slots[domain]:
			current_round['domain'] = domain
			current_round['slot'] = slot
			num_inform_slot = data2num_inform_slot([current_round])
			prediction = self.sess.run(self.inform_slot_tracker[domain][slot]._probability,
			                      feed_dict={
				                      self.inform_slot_tracker[domain][slot]._x_user_nl: num_inform_slot['user_nl'],
				                      self.inform_slot_tracker[domain][slot]._x_stringmatch: num_inform_slot[
					                      'stringmatch'],
				                      self.inform_slot_tracker[domain][slot]._x_sys_act: num_inform_slot['sys_act'],
				                      self.inform_slot_tracker[domain][slot]._is_training: False})
			del num_inform_slot
			...
			if informed_slots[domain][slot] == "MENTIONED":
				...
				prediction = self.sess.run(self.value_tracker[domain][slot]._probability,
				                      feed_dict={
					                      self.value_tracker[domain][slot]._x_user_nl: num_value['user_nl'],
					                      self.value_tracker[domain][slot]._x_stringmatch: num_value['stringmatch'],
					                      self.value_tracker[domain][slot]._is_training: False})
				del num_value
	...
	def slu(self, current_round, last_domain):
		...
		if 'inform_some_match' in sys_act:
			detected_user_acts = self.evaluate_user_act(current_round, detected_user_acts)
			detected_user_acts = list(
				filter(lambda x: x in ['first', 'second', 'third', 'last', 'other'], detected_user_acts))
			major_key = {'电影': '片名', '音乐': '歌名'}
			for act in detected_user_acts:
				if act in ['first', 'second', 'third', 'last', 'other']:
					domain = last_domain
				if act == 'first':
					major_key_mem[domain][major_key[domain]] = [
						sys_act['inform_some_match'][domain][major_key[domain]][0]]
				if act == 'second':
					major_key_mem[domain][major_key[domain]] = [
						sys_act['inform_some_match'][domain][major_key[domain]][1]]
				if act == 'third':
					index = min(len(sys_act['inform_some_match'][domain][major_key[domain]]), 3)
					major_key_mem[domain][major_key[domain]] = [
						sys_act['inform_some_match'][domain][major_key[domain]][index - 1]]
				if act == 'last':
					major_key_mem[domain][major_key[domain]] = [
						sys_act['inform_some_match'][domain][major_key[domain]][-1]]
		# domain
		if domain is None:
			domain = self.evaluate_domain(current_round, last_domain)
		# requested_slots
		requested_slots = {domain: []}
		requested_slots = self.evaluate_request_slot(current_round, domain, requested_slots)
		# major_key
		if domain in ['电影', '音乐', '天气']:
			major_key_mem = self.evaluate_major_key(current_round, domain, major_key_mem)
		# informed_slots, confirmed_slots
		informed_slots = {domain: {}}
		if domain in ['电影', '音乐']:
			informed_slots = self.evaluate_inform_slot(current_round, domain, informed_slots)
		...
		current_belief_states = {'major_key': major_key_mem, 'informed': informed_slots,
		                         'requested': requested_slots}
		current_domain = domain
		return current_belief_states, current_domain, detected_user_acts
	
	def update_belief_states(self, current_round, current_belief_states, current_domain, last_belief_states):
		sys_act = current_round['sys_act']
	...
	def update(self, input_usr):
		input_usr = input_usr.lstrip('嗯')
		if input_usr in ["重来", "谢谢", "结束"]:  # 用户重来
			print('\n\n')
			self.system_init()
			sys_nl = "欢迎使用清华大学电子系多领域信息问询系统，您可以对电影、音乐、时间、天气信息进行查询。"
			self.dialog_history.append(("usr: " + input_usr, "sys: " + sys_nl))
			...
			current_round["sys_act"] = self.sys_act
			
			# 计算 belief states
			current_belief_states, current_domain, detected_user_acts = self.slu(current_round, self.last_domain)
			# print(current_belief_states, current_domain, detected_user_acts)
			belief_states = self.update_belief_states(current_round, current_belief_states, current_domain,
			                                          self.last_belief_states)
			self.last_belief_states = copy.deepcopy(belief_states)
			self.last_domain = copy.deepcopy(current_domain)
			...
			# 生成 system_acts
			self.sys_act, self.KB_pointer = rule_based_policy(belief_states, current_domain, self.KB_pointer, knowledge_base)
			# 生成回复
			sys_nl = rule_based_NLG(self.sys_act, self.KB_pointer, knowledge_base)
			self.dialog_history.append(("usr: " + input_usr, "sys: " + sys_nl))
			return self.dialog_history
